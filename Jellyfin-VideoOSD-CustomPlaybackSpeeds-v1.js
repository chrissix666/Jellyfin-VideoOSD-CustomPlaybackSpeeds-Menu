...
(function () {
    ...
    function isSpeedMenu(sheet) {
        ...
        const ids = [...scroller.querySelectorAll("button[data-id]")].map(b => b.getAttribute("data-id"));
    ...
    function patch(sheet) {
        ...
        const originalCount = [...scroller.querySelectorAll("button[data-id]")].map(b => parseFloat(b.getAttribute("data-id"))).filter(v => !Number.isNaN(v)).length;
        scroller.querySelectorAll("button[data-id]").forEach(button => {
            const value = parseFloat(button.getAttribute("data-id"));
            if (!Number.isNaN(value)) ensureCheckSlot(button);
        });
        const template = scroller.querySelector('button[data-id="0.5"]') || scroller.querySelector('button[data-id="1"]') || scroller.querySelector("button[data-id]");
        if (!template) return;
        ...
        [...scroller.querySelectorAll("button[data-id]")].map(b => ({ el: b, id: parseFloat(b.getAttribute("data-id")) })).filter(x => !Number.isNaN(x.id)).sort((a, b) => a.id - b.id).forEach(item => scroller.appendChild(item.el));
